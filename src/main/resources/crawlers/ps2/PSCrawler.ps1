...
$json = "{`"Hostname`": `"" + $env:computername + "`", `"IP`": `"" + $computerInfo.IPV4Address.IPAddressToString + "`", `"TimeExecuted`": `"" + (Get-Date)  + "`", `"Query`": " + $query + "}"
$jsonProcess = $json.Replace("\", "\\")
$jsonProcessEscape = $jsonProcess.Replace("`"", "\```"")

$script = ".\curl.exe $execute -H `"Accept: application/json`" -H `"Content-Type: application/json`" -X POST -d `"$jsonProcessEscape`""
iex $script